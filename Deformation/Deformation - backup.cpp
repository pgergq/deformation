...
std::ofstream						debug;
std::vector<std::vector<float>>		g_vVertices;					// file import data
std::vector<std::vector<int>>		g_vFaces;						
...
HRESULT CreateParticleBuffer( ID3D11Device* pd3dDevice )
{
	...
	OutputDebugString(L"<CreateParticleBuffer");
    return hr;
}
...
void ReadModel()
{
	...
	// Import file
	std::ifstream input;
	input.open("bunny_res3_scaled.obj");
...
void LoadVolumetricCubes(MASSPOINT* volcube1, MASSPOINT* volcube2, PARTICLE* particles)
{
	...
	OutputDebugString(L"<LoadVolCubes");
}
...
HRESULT CreateParticlePosVeloBuffers( ID3D11Device* pd3dDevice )
{
	...
	//sdesc.BindFlags = 0;
	//sdesc.ByteWidth = VOLCUBE1_COUNT * sizeof(MASSPOINT);
	//sdesc.MiscFlags = D3D11_RESOURCE_MISC_BUFFER_STRUCTURED;
	//sdesc.StructureByteStride = sizeof(MASSPOINT);
	//V_RETURN(pd3dDevice->CreateBuffer(&sdesc, nullptr, &g_pDebugBuffer));
	//DXUT_SetDebugName( g_pDebugBuffer, "DebugBuffer" );
	...
	OutputDebugString(L"<CreateParticlePVBuffers");
    return hr;
}
                                         ...
                                         const DXGI_SURFACE_DESC* pBackBufferSurfaceDesc, void* pUserContext )
{
	...
	OutputDebugString(L"<ResizedSwap");
    return hr;
}
...
void ModelRenderBuffers(ID3D11Device* pd3dDevice, int width, int height)
{
	...
	OutputDebugString(L"<ModelRenderBuffers");
...
bool RenderModel( ID3D11DeviceContext* pd3dImmediateContext, CXMMATRIX mView, CXMMATRIX mProj )
{
	...
	OutputDebugString(L"<RenderModel");
    return true;
}
